...
export async function requireApiKey(req: Request) {
  ...
  for (const keyDoc of candidates) {
    ...
    if (ok) {
      ...
      // ✅ BYO Redis safety check
      if (
        !namespace.redisUrlEnc ||
        !namespace.redisUrlIv ||
        !namespace.redisUrlTag
      ) {
        throw new Error("Namespace Redis config missing");
      }

      // ✅ SaaS ownership check (VERY IMPORTANT)
      if (keyDoc.ownerUserId !== namespace.ownerUserId) {
